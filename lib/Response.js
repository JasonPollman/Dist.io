...
/**
 * A simple response abstraction object that represents a request's response.
 * @constructor
 */
class Response {
  /**
  ...
  constructor(message) {
    ...
    this[response].raw = message._.copy();
  ...
  /**
   * Returns the response as a "raw" object so it can be transmitted.
   * Used by the MasterProxyServer to send back responses to the remote master client.
   * @return {Object} The raw response, formatted to transmission.
   */
  get raw() {
    return this[response].raw._.clone();
  }
   ...
   */
  get slave() {
    return require('./Slave').getSlaveWithId(this[response].request.for); // eslint-disable-line global-require
  }
   ...
   */
  get request() {
    return this[response].request._.copy();
  }
 ...
 */
  ...
  /**
   * Sets the error associated with this response, if any.
   * @param {*} v The value to set.
   */
  set error(v) {
