...
function initSlave(server, socket, log, id, loc, options) {
  ...
  queuedSlaveTasks[socket.id].forEach(fn => {
    log.verbose(`Executing queued task (${queuedSlaveTasks[socket.id].length} pending)...`);
    fn();
  });
...
function executeTask(log, m, socket, id, respond) {
  ...
  if (slave && Slave.isMasterMessage(m)) {
    log.verbose(
    ...
    if (killMatch) { // Request to forcefully kill the slave...
      ...
      switch (m.command) {
        ...
        case '__dist.io__info__': m.command = commands.INFO; break;
        default: /* Noop */
      }
    ...
    respond.with.error(m.for, new ReferenceError('Invalid request'));
    socket.disconnect();
  }
}
  ...
  socket.on('message', m => {
    ...
    if (args) {
      ...
      if (master.slaveCount < server.maxConcurrentSlaves) {
        ...
        log.verbose(
          `Queuing slave with remote id ${args['dist-io-slave-id']} ` +
          `(current slave count is ${master.slaveCount}, max is ${server.maxConcurrentSlaves})`
        );
        queuedSlaves.push(() => {
          initSlave(server, socket, log, args['dist-io-slave-id'], m.path, m.options);
        });
      ...
      if (!slave) {
        if (!Slave.isMasterMessage(m)) {
          log.error(`Client @${socket.handshake.address} send an invalid/malformed request, disconnecting.`);
          respond.with.error(m.for, new ReferenceError('Invalid request'));
        } else if (typeof remoteToLocalSlaveIdMap[socket.id][m.for] !== 'number') {
