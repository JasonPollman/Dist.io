    ...
    if (args) {
      ...
      if (Slave.isMasterMessage(m)) {
        log.verbose(
          ...
          `executing task "${m.command}" with slave S${serverId} (R${m.for}).`
        );
        ...
        if (killMatch) { // Request to forcefully kill the slave...
          log.warn(`${socket.handshake.address} has killed (${killMatch[1]}) slave S${serverId} (R${m.for}).`);
          ...
          slave.exec(m.command, m.data, m.meta)
            .then(res => {
              ...
              switch (newCommand) {
                case commands.EXIT:
                  log.close(`${socket.handshake.address} has closed slave S${serverId} (R${m.for}).`);
                  delete remoteToLocalSlaveIdMap[socket.id][m.for];
