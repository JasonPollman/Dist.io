...
const Commands = require('./PrivateCommands');
...
function isSlaveExceptionMessage(m) {
  if (m.sent && m.title === 'SlaveIOException' && typeof m.error === 'object') return true;
  return false;
}
  ...
  if (typeof m === 'object') {
    if (isSlaveMessage(m)) {
      if (pendingRequests[m.request.for] && pendingRequests[m.request.for][m.request.rid]) {
        const rawResponse = pendingRequests[m.request.for][m.request.rid];
        rawResponse.clearTimeout();
        ...
        if (rawResponse.hasTimedout(response)) {
          // Ignore the message, no op...
        } else if (rawResponse.callback instanceof Function) {
          // Invoke the callback for the message.
          rawResponse.callback.call(response, response.error, response.data, response);
        }
      }
    } else if (isSlaveExceptionMessage(m)) {
      const e = new Error(m.error.message || 'Unknown error');
      if (m.error.stack) e.stack = m.error.stack;
      if (slaves[m.for]) slaves[m.for][settings].onUncaughtException(e);
    }
  }
}
...
function sendCommandMessageToChildWithMetadata(command, slave, meta, data, done) {
  ...
  request.onTimeout(() => {
    ...
    request.callback.call(response, response.error, response.data, response);
...
class Slave extends EventEmitter {
  ...
  constructor(file, opts) {
    ...
    if (!file || typeof file !== 'string') {
      throw new ReferenceError(`Slave constructor argument #0 requires a string, but got: ${typeof file}`);
    }
    ...
    options.args.push(`--master-io-slave-id=${this[settings].id}`, `--master-io-slave-alias=${this[settings].alias}`);

    // Fork the child process...
    this[settings].process = fork(file, options.args, options.forkOptions || { stdio: 'pipe' });
    initializeSlaveEventsWithSlaveAndChildProcess(this, this[settings].process, opts);
  }
  ...
  /**
   * [onUncaughtException description]
   ...
   */
  toString() {
    return `Slave (id=${this[settings].id}, alias=${this[settings].alias})`;
  }
  ...
  close(...args) {
    ...
    if (!this[settings].spawnError && !this[settings].exited && !this[settings].closed) {
      return new Promise((resolve, reject) => {
        ...
        sendCommandMessageToChildWithMetadata(Commands.EXIT, this, null, null, (err, status) => {
          this[settings].gotExitResponse = true;
          cleanupSlaveReferences(this);
          done.call(this, err, status);
          return err ? reject(err) : resolve(status);
        });
      });
    }
    return this;
      ...
      if (!this[settings].spawnError && !this[settings].exited && !this[settings].closed) {
        ...
        sendCommandMessageToChildWithMetadata(command, this, meta, data,
          ...
          function onResponse(err, response) {
            ...
            resolve(this);
            done.call(this, null, response, this);
          });
