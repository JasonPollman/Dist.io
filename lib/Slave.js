...
class Slave extends EventEmitter {
  ...
  static getAllIdleSlaves() {
    return slaves._.toArray().filter(s => s.isIdle);
  }
  ...
  static getAllBusySlaves() {
    return slaves._.toArray().filter(s => s.isBusy);
  }
  ...
  static getLeastBusy(...slaveList) {
    const slavesToEval = [...slaveList];
    if (slavesToEval.length === 0) return null;
    return slavesToEval._.min(s => (s instanceof Slave ? s.pendingRequests : Number.MAX_VALUE));
  }
  ...
  then(cb) {
    return new Promise(resolve => {
      if (cb instanceof Function) cb.call(this, this);
      resolve();
    });
  }
