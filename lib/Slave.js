...
class Slave extends EventEmitter {
  ...
  constructor(loc, opts) {
    ...
    if (!parsedArgs['dist-io-slave-id'] && parsedArgs['dist-io-slave-id'] !== 0) {
      args.push(`--dist-io-slave-id=${this[settings].id}`);
    }
  ...
  /**
   * @return {Function} This slave's onUncaughtException handler.
   */
  get onUncaughtException() {
    return this[settings].onUncaughtException;
  }
      ...
      if (canInteract(this)) {
        ...
        sendCommandMessageToChildWithMetadata(command, this, meta, data,
          ...
          (response, request) => {
            ...
            if (typeof response.error === 'object' && meta.catchAll) {
