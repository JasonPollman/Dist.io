...
function cleanupSlaveReferences(slave) {
  if (slave[settings].process.unref instanceof Function) slave[settings].process.unref();
...
function initializeSlaveEventsWithSlaveAndChildProcess(slave, child, options) {
  ...
  child.on('error', function onSpawnError(e) { // eslint-disable-line prefer-arrow-callback
    ...
    if (slave[settings].onSpawnError instanceof Function) slave[settings].onSpawnError.call(slave, e);

    // Add a new generic error listener
    child.on('error', () => {
      cleanupSlaveReferences(slave);
      if (slave[settings].onUncaughtException instanceof Function) slave[settings].onUncaughtException.call(slave, e);
    });
  });
...
class Slave extends EventEmitter {
  ...
  constructor(loc, opts) {
    ...
    this[settings] = {
      ...
      /**
       * Called if a child fails to spawn.
       * @type {Function}
       */
      onSpawnError: options.onSpawnError instanceof Function ? options.onSpawnError : null,
    };
   ...
   * @return {undefined}
   */
  onUncaughtException(f) {
    if (typeof f === 'function') this[settings].onUncaughtException = f;
  }

  /**
   * The function to invoke if the child fails to spawn
   * @param {Function} f A callback function.
   * @return {undefined}
   */
  onSpawnError(f) {
    if (typeof f === 'function') this[settings].onSpawnError = f;
  }
      ...
      if (canInteract(this)) {
        ...
        sendCommandMessageToChildWithMetadata(command, this, meta, data,
          ...
          (response, request) => {
            ...
            if (typeof response.error === 'object' && meta && meta.catchAll) {
