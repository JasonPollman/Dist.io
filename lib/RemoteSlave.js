...
let DEFAULT_SPAWN_ERROR = e => { throw e; };
...
function initializeSlaveEventsWithSlaveAndPseudoChildProcess(slave, args, socket, options) {
  ...
  socket.on('reconnect_failed', () => {
    if (!slave[remoteslave].handshake) {
      setSlaveExitedToTrue();
      const e = new Error(`Unable to connect to host @${slave.location}`);
      // Call user onSpawnError function...
      if (slave[slave[remoteslave].protected].onSpawnError instanceof Function) {
        slave[slave[remoteslave].protected].onSpawnError.call(slave, e);
      }
    }
  ...
  // Should a re-connection fail, cleanup references and call the onSpawnError handler.
  socket.on('spawn failed', m => {
    setSlaveExitedToTrue();
    // Call user onSpawnError function...
    if (slave[slave[remoteslave].protected].onSpawnError instanceof Function) {
      slave[slave[remoteslave].protected].onSpawnError.call(slave, new Error(m));
    }
  });
  ...
  socket.on('error', e => {
    ...
    if (slave[slave[remoteslave].protected].onUncaughtException instanceof Function) {
      slave[slave[remoteslave].protected].onUncaughtException.call(slave, e);
    }
  });
...
class RemoteSlave extends Slave {
  ...
  /**
   * The default onSpawnError handler.
   * @param {Function} f The function to set as the default onSpawnError.
   * @return {undefined}
   */
  static onSpawnError(f) {
    if (f instanceof Function) DEFAULT_SPAWN_ERROR = f;
  }
  ...
  constructor(connectOptions, opts) {
    ...
    if (!(options.onSpawnError instanceof Function)) {
      options.onSpawnError = DEFAULT_SPAWN_ERROR;
    }
  ...
  slaveResponseListener(m) {
    ...
    if (this.isSlaveMessage(m)) {
      ...
      if (this.id === m.from) {
        ...
        if (this[this[remoteslave].protected].onUncaughtException instanceof Function) {
          this[this[remoteslave].protected].onUncaughtException(e);
        }
      }
    }
  }
