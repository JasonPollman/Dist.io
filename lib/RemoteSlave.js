...
class PseduoChildProcess {
   ...
   */
  get pid() {
    return this[pseduo].slave[remoteslave].remotePID;
  }
  ...
  kill(signal) {
    this[pseduo].slave.exec(Commands.REMOTE_KILL(signal || 'SIGKILL'), null, null) // eslint-disable-line new-cap
      .then(res => {
        this[remoteslave].isConnected = false;
        Slave.cleanupSlaveReferences(this);
   ...
   */
  send(m) {
    this[pseduo].slave[remoteslave].socket.emit('message', m);
    return this;
  }
}
...
function initializeSlaveEventsWithSlaveAndPseudoChildProcess(slave, args, socket, options) {
  ...
  socket.on('spawn failed', m => {
    ...
    if (slave[slave[remoteslave].protected].onSpawnError instanceof Function) {
      slave[slave[remoteslave].protected].onSpawnError.call(slave, new Error(
        `Failed to connect to remote slave @${slave.location}:${os.EOL}${m}`
      ));
    }
  });
...
class RemoteSlave extends Slave {
  ...
  static set reconnectionDelay(r) {
    if (r._.isNumeric()) RECONNECTION_DELAY = r._.getNumeric();
  }
  ...
  initProcess(loc, options, args) {
    ...
    if (!/^http(s)?:\/\//.test(this[remoteslave].connectionString)) {
      this[remoteslave].connectionString = `http://${this[remoteslave].connectionString}`;
    }
