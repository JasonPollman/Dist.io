...
function initializeSlaveEventsWithSlaveAndPseudoChildProcess(slave, args, socket, options) {
  const setSlaveExitedToTrue = () => Slave.cleanupSlaveReferences(slave);
  ...
  socket.on('remote slave exited', () => { slave[slave[remoteslave].protected].exited = true; });
  socket.on('remote slave closed', () => { slave[slave[remoteslave].protected].closed = true; });
