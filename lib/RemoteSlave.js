...
function initializeSlaveEventsWithSlaveAndPseudoChildProcess(slave, args, socket, options) {
  ...
  socket.on('reconnect_failed', () => {
    ...
    if (!slave[remoteslave].handshake && !handledSpawnError) {
      ...
      if (slave.listenerCount('spawn error') === 0 && socket.listeners('reconnect_failed').length === 1) {
        throw e;
      } else {
  ...
  // If the remote closes or is killed, ensure we do the same local.
  socket.on('remote slave exited', () => {
    setSlaveExitedToTrue();
    slave.emit('exited');
  });

  socket.on('remote slave killed', () => {
    setSlaveExitedToTrue();
    slave.emit('killed');
  });

  socket.on('remote slave closed', () => {
    setSlaveExitedToTrue();
    slave.emit('closed');
  });
  ...
  socket.on('remote pid', id => {
    /**
     * Emitted when the slave has been spawned by the master proxy server and the process id is now available.
     * This establishes a "handshake" between the server and this process, that indicates the slave is ready for work.
     * @event remote handshake
     * @argument {Number} id The slave's process id
     */
    slave.emit('remote handshake', id);
  ...
  socket.on('error', e => {
    if (slave[remoteslave].handshake) { // Already spawned handle as error...
      setSlaveExitedToTrue();
      if (slave.listenerCount('uncaughtException') === 0 && socket.listeners('error').length === 1) {
        // Throw an error if and only if a user has *not* subscribed to the "uncaughtException" event
        // and the user doesn't already have a socket.on('error') lietener installed.
        throw e;
      } else {
        slave.emit('uncaughtException', e);
      }
    } else { // Not yet spawned, handle as spawn error...
      onSpawnError(e);
    }
  });
...
class RemoteSlave extends Slave {
  ...
  /**
   * @return {String} The slave's host location.
   */
  get location() {
    return this[remoteslave].connectionString;
  }
  ...
  initProcess(loc, options, args) {
    ...
    this[remoteslave].connectionString = `${location[1]}://${location[4]}:${location[5]}`;
    ...
    if (this[remoteslave].username && this[remoteslave].pass) {
      ...
      if (this[remoteslave].passphrase) {
        ...
        socketOptions.extraHeaders = { authorization: `${encrypted}` };
      } else {
        socketOptions.extraHeaders = { authorization: `${new Buffer(authString).toString('base64')}` };
      }
    }
