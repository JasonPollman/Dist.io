...
const RemoteSlave = require('./RemoteSlave');
 ...
 * Controls IO flow and sends messages to local and remote slave processes.
...
class Master extends EventEmitter {
  ...
  get slaves() {
    return {
       ...
       */
      local: new SlaveArray(...Slave.getAllSlaves()._.where(s => (s.isRemote === false))),

      /**
       * @return {SlaveArray} All remote slaves.
       */
      remote: new SlaveArray(...Slave.getAllSlaves()._.where(s => (s.isRemote === true))),
