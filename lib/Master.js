...
class Master extends EventEmitter {
  ...
  get slaves() {
    return {
       ...
       */
      idleInList: (...slaveList) => Slave.getAllIdleSlaves(...flattenSlaveList(true, ...slaveList)),
      ...
      notInGroup: (g) => {
        ...
        return all.filter(s => slavesInGroupG.indexOf(s) === -1);
      },
  ...
  createSlave(path, slaveOptions, ...rest) {
    return new Slave(path, slaveOptions, ...rest);
  }
   ...
   */
  get tell() {
    return (...slaves) => ({ to: (...args) => this.broadcast(...args).to(...slaves) });
  }
  ...
  createParallel() {
        ...
        const doIteration = (n) => {
          tasks.forEach((t, i) => {
            ...
            if (!(t.slave instanceof Slave)) {
              const e = new TypeError(`Task #${i} is missing a slave. Did you forget to call ".with"?`);
              done.call(this, e, null);
              reject(e);
              return;
            }
    ...
    withSlave = (slave) => {
      slave = Slave.getSlave(slave);
      if (!slave) throw new TypeError('Master#paralle.do.with expected argument #0 to be an instanceof Slave.');

      tasks[tasks.length - 1].slave = slave;
      return { do: doTask, execute: executeTasks, times: setTimes };
    };

    doTask = (command, data, meta) => {
      tasks.push({ command, data, meta, slave: null });
      return { with: withSlave };
    };

    return { do: doTask, execute: executeTasks };
  }
  ...
  createWorkpool(...slaves) {
    ...
    /**
     * Gets the next slave to perform a task.
     * This function first finds an idle slave. If an idle slave is found, it is returned.
     * If not, it uses the next slave in the list (next in comparison from the last call).
     * @return {Slave} The next available slave in the list.
     */
    const getNextSlave = () => {
      if (lastIndex === slaves.length - 1) lastIndex = -1;

      let slave = this.slaves
        .idleInList(...slaves)
        ._.where(s => slaves.indexOf(s) > lastIndex)
        ._.first();

      if (slave) {
        lastIndex = slaves.indexOf(slave);
      } else {
        slave = slaves[++lastIndex];
      }
      return slave;
    };
    ...
    return {
      ...
      /**
       * Performs a workpool task until the predicate function returns false.
       * @param {[type]} list The argument list.
       * @return {[type]} [description]
       */
      while: (...list) => {
        const args = [...list];
        const predicate = args._.getCallback();

        /**
         * If true, the workpool will wait for one response before starting the next.
         * @type {Boolean}
         */
        let wait = false;

        if (typeof args[0] !== 'function') wait = !!args[0];
        if (!(predicate instanceof Function)) {
          throw new TypeError(
            `Master#createWorkpool.while expected argument #0 (predicate) to be a function, but got ${typeof predicate}`
          );
        } else {
          return {
            /**
             * Performs the following command x times.
             * @param {String|Symbol} command The command to perform.
             * @param {*} data The data to execute the command with.
             * @param {Object} meta The metadata to send with the data.
             * @param {...*} rest The rest of the arugment list.
             * @return {Promise} A promise for completion.
             */
            do: (command, data, meta, ...rest) => new Promise((resolve, reject) => {
              const responses = new ResponseArray();
              const done = [...rest]._.getCallback();

              let i = 0;

              if (wait) { // Wait for one response before sending the next...
                const doTask = () => {
                  if (predicate.call(this, i, responses._.last())) {
                    getNextSlave()
                      .exec(command, data, meta, ...rest)
                      .then(res => {
                        i++;
                        responses.push(res);
                        doTask();
                ...
                doTask();
              } else { // Send all requests in parallel.
                let completed = 0;
                let broken = false;
                let predicateResult;
                ...
                const onResult = res => {
                  responses.push(res);
                  if (++completed === i && !predicateResult) {
                    done.call(this, null, responses);
                    resolve(responses);
                  }
                };
                ...
                /**
                 * Invoked when an error is thrown by the request.
                 * @param {Error} e The error the request threw.
                 * @return {undefined}
                 * @function
                 */
                const onError = e => {
                ...
                predicateResult = predicate.call(this, i);
                if (!predicateResult) {
                  done.call(this, null, responses);
                  resolve(responses);
                  return;
                ...
                while (predicateResult && !broken) { // eslint-disable-line no-cond-assign
                  getNextSlave()
                    .exec(command, data, meta, ...rest)
                    .then(onResult)
                    .catch(onError);
      ...
      do: (command, data, meta, ...rest) => this.slaves.leastBusyInList(...slaves).exec(command, data, meta, ...rest),
    };
  }
  ...
  broadcast(command, ...args) {
    ...
    bcast.to.idle = () => this.broadcast(command, ...args).to(...Slave.getAllIdleSlaves());
    // Attach broadcast to group function
    bcast.to.group = (g) => this.broadcast(command, ...args).to(...Slave.getSlavesInGroup(g));
    return bcast;
  }
  ...
  createPipeline() {
     ...
     * @return {Promise} A promise for completion.
     */
    const execute = (data, meta, ...rest) => new Promise((resolve, reject) => {
      ...
      const doTask = (taskToPerform, dataForTask, metadataForTask) => {
        if (!(taskToPerform.slave instanceof Slave)) {
          reject(`Task ${taskToPerform} is missing a slave. Did you forget to chain ".with"?`);
          return;
        }
  ...
  createScatter(command, meta) {
     ...
     * @param {...Slaves} s The slaves to execute this scatter with.
