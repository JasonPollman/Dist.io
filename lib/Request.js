...
class Request {
  ...
  constructor(slave, command, meta, data, secretId, secretNumber, slaveSymbol, callback) {
    ...
    if (!(slave instanceof require('./Slave'))) { // eslint-disable-line global-require
      throw new Error('Request argument #0 (slave) expected a an instanceof Slave');
    } else if (typeof secretId !== 'string') {
      throw new Error(`Request argument #4 (secretId) expected a string, but got ${typeof secretId}`);
    } else if (typeof secretNumber !== 'number') {
      throw new Error(`Request argument #5 (secretNumber) expected a string, but got ${typeof secretNumber}`);
    } else if (typeof meta !== 'object') {
      throw new Error(`Request argument #2 (meta) expected an object, but got ${typeof object}`);
    }
    ...
    this[request] = {
       ...
       * @type {Number}
       */
      created: created[0] * 1e9 + created[1],
    ...
    if (timeout && timeout._.isNumeric()) {
      ...
      this[request].timeout = setTimeout(() => this.onTimeout(), timeout);
    }
  }
  ...
  hasTimedout(response) {
    ...
    if (this.timeout && (this.created + this.ttl) > (response.received - this.created)) return true;
    return false;
  }
