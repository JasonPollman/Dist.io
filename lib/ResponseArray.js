...
function validateArgumentsAreAllResponses(...responses) {
  responses.forEach(r => {
    if (!(r instanceof Response) && !(r instanceof ResponseError)) {
      throw new TypeError('Cannot insert non-Response object into ResponseArray!');
    }
  });
}
...
class ResponseArray extends Array {
  /**
   * @param {...Response} responses A list of response object to add to the ResponseArray.
   ...
   * @param {String} order If 'desc', it will sort in descending order, ascending otherwise.
   * @return {ResponseArray} The sorted response array.
   */
  sortBy(prop, order) {
    if (this.length > 0) {
      if (this[0][prop] !== undefined) {
        if (order === 'desc') {
        ...
        throw new TypeError(`Response has no property "${prop}"`);
      }
    }
    return this;
  }
   ...
   */
  get product() {
    return this.values.reduce((p, c) => p * c, 1);
  }
