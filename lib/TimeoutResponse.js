/**
 * @file
 * A response that's sent when a request times out.
 * @copyright © 2016 Jason James Pollman
 */

'use strict';
/**
 * @module DistIO/TimeoutResponse
 */
...
class TimeoutResponse extends Response {
  ...
  constructor(slave, request) {
    ...
    super({
      ...
      sent: Date.now(),
      request,
      error: new Error(`Request #${request.rid} with command "${request.command}" timed out after ${request.ttl}ms.`),
      data: null,
    });
  }
}
